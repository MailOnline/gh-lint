...
const {allUsers, committer, plural, users} = require('./util');
...
module.exports = {
  ...
  async check(cfg, repoCommits, orgRepo, github) {
    ...
    for (const cmt of repoCommits) {
      const excluded = exclCommitters && exclCommitters.indexOf(committer(cmt)) >= 0;
    ...
    return {
      valid: false,
      message: `${commits.length} commit${plural(commits)} without PR by ${allUsers(commits)}`,
