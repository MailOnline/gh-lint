  ...
  async prepareRepoRules (config, options={}) {
    ...
    async function addTeams() {
      ...
      for (const teamName in teams) {
        ...
        const repos = await github.getRepos.team(orgName, shortTeamName, options.teamPermission);
...
function prepareOptions(options) {
  ...
  const {after, before, teamPermission} = options;
  ...
  if (!teamPermission) options.teamPermission = 'admin';
  return options;
}
